        ...
        for f, frame in enumerate(d["frames"]):
            ...
            for feature in phase_features:
                if feature == "phase_vec":
                    sin = np.asarray([d["frames"][idx][jj]["phase_vec"] for jj in [14, 20] for idx in f_idx])
                    vel = np.concatenate([d["frames"][idx][jj]["velocity"] for jj in [14, 20] for idx in f_idx])
                    vel = np.reshape(vel, (3,-1))
                    vel = np.sqrt(np.sum(vel**2, axis=0))
                    vel[vel==0] = 1
                    cos = np.cos(np.arcsin(sin/vel)) * vel
                    cos = np.nan_to_num(cos, 0)
                    row_vec.append(np.concatenate([np.asarray([sin[i], cos[i]]) for i in range(len(sin))]))
                else:
                    row_vec.append(np.asarray([frame[jj][feature] for jj in [14, 20]]))
            ...
            for feature in cost_features:
                if feature == "contact":
                    row_vec.append(np.asarray([d["frames"][idx][jj]["contact"] for jj in [14, 20] for idx in f_idx]))
                elif feature == "targetRotation":
                    row_vec.append(np.concatenate([frame[jj][feature].ravel() for jj in [14, 20]]))
                elif feature == "posCost" or feature == "rotCost":
                    row_vec.append(np.concatenate([d["frames"][idx][jj][feature] for jj in [14, 20] for idx in f_idx]))
                else:
                    row_vec.append(np.concatenate([frame[jj][feature] for jj in [14, 20]]))
