...
class MotionGenerationModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.autoregress_chunk_size == 1:
            out = self(x.squeeze(dim=0))
            loss = self.loss_fn(out, y.squeeze(dim=0))
        else:
            x_chunks = torch.split(x, self.autoregress_chunk_size, dim=1)
            y_chunks = torch.split(y, self.autoregress_chunk_size, dim=1)
            loss = 0
            n_chunks = float(len(x_chunks))
            for xc, yc in zip(x_chunks, y_chunks):
                chunk_loss = 0
                x_f = xc[0][0].unsqueeze(dim=0)  # first frame in each chunk
                for yf in yc[0]:
                    out_f = self(x_f)
                    loss_f = self.loss_fn(out_f, yf.unsqueeze(dim=0))
                    chunk_loss += loss_f
                    x_f = out_f
                loss += chunk_loss
            loss /= n_chunks
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.autoregress_chunk_size == 1:
            ...
            loss /= n_chunks
            if torch.isnan(loss):
                raise ValueError("Loss is nan, ", n_chunks, chunk_loss.item())
