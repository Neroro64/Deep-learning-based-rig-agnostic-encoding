...
from MLP_Adversarial import MLP_ADV as MLP
...
class MotionGenerationModel(pl.LightningModule):
    def __init__(self, config:dict=None, Model=None, pose_autoencoder=None, middle_layer=None, feature_dims=None,
        ...
        self.cost_encoder = MLP(config=config,h_dim=100, w_dim=100,
        ...
        self.reduced_set = config["use_label"]
        if self.reduced_set:
            self.pose_label = config["pose_label"]
    ...
    def forward(self, x):
        ...
        extra_loss = self.middle_layer.loss(pose_h)

        return [phase, new_pose, out_tensors[-1], x_tensors[-1]], extra_loss
    ...
    def step(self, x, y):
        ...
        x_c = x[:, 0, :]
        predicted = [x_c[:, self.in_slices[1]:self.in_slices[2]].detach().unsqueeze(1)]
        ...
        for i in range(1, n):
            ...
            tot_extra_loss += extra_loss.detach()
            ...
            opt.zero_grad()
            self.manual_backward(loss+extra_loss)
            opt.step()

            out[1] = out[1][:, self.pose_label]
        ...
        tot_extra_loss /= float(n)
        return tot_loss, tot_pos_loss, tot_rot_loss, predicted, tot_extra_loss,\
    ...
    def step_eval(self, x, y):
        ...
        for i in range(1, n):
            ...
            _, pos_loss, rot_loss = self.pose_autoencoder.loss(out[1],
                                                               y_c[:, self.phase_dim:self.phase_dim + self.pose_dim])
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, tot_pos_loss, tot_rot_loss, predicted, tot_extra_loss,\
               min_pos_cost, min_rot_cost, min_target_pos_cost, min_target_rot_cost,\
        ...
        self.log("ptl/train_C_loss", tot_extra_loss)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("ptl/val_C_loss", tot_extra_loss,prog_bar=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("ptl/test_C_loss", tot_extra_loss,prog_bar=True)
