...
class MoE(nn.Module):
    def __init__(self, config=None, dimensions=None,
        ...
        layers = [
            (
                nn.Parameter(torch.empty(self.k_experts, self.dimensions[0]+self.z_dim, self.hidden_dim)),
                nn.Parameter(torch.empty(self.k_experts, self.hidden_dim)),
                self.act_fn()
            ),
            (
                nn.Parameter(torch.empty(self.k_experts, self.hidden_dim+self.z_dim, self.hidden_dim)),
                nn.Parameter(torch.empty(self.k_experts, self.hidden_dim)),
                self.act_fn()
            ),
            (
                nn.Parameter(torch.empty(self.k_experts, self.hidden_dim+self.z_dim, self.dimensions[-1])),
                nn.Parameter(torch.empty(self.k_experts, self.dimensions[-1])),
                None
            ),
        ]
        self.layers = layers
