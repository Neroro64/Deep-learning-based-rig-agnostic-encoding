...
class LSTM(nn.Module):
    def __init__(self, config=None, dimensions=None, phase_input_dim:int=0, name="model", device="cuda"):
        ...
        self.dimensions = dimensions if len(dimensions) > 2 else \
            [dimensions[0]+phase_input_dim+self.z_dim, self.hidden_dim, self.hidden_dim, dimensions[-1]]
    ...
    def forward(self, x:torch.Tensor, c:torch.Tensor, z:torch.Tensor) -> torch.Tensor:
        x = torch.cat((z,c,x), dim=1).unsqueeze(dim=1)
    ...
    def reset_hidden(self, batch_size=0):
        ...
        cell_state = torch.autograd.Variable(torch.randn(self.num_layers, batch_size, self.hidden_dim, device=self.device))
    ...
    def load_checkpoint(filePath):
        ...
        model = LSTM(config=obj["config"], name=obj["name"], dimensions=obj["dimensions"], device=obj["device"],
