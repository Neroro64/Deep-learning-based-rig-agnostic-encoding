...
def train_model(config, name,featureDim, feature_dims,
    ...
    pretrained_model = "G:/MEX/models/RBF_MoE_256_ZCAT0_F_R1_ZCAT/final.pbz2"
    ...
    c_layer_2 = C(config=config, input_dims=[372]).cluster_model if C is not None else MLP_layer()
    ...
    try:
        for dname, dirs, files in os.walk("G:/MEX/datasets"):
            for i, file in enumerate(files):
                if "L" in file or "1_F" in file or "2_F" in file or "R1" in file or "R2" in file:
                    continue
