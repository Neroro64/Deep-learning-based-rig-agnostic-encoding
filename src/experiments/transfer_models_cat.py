...
sys.path.append("../motion_generation_models_models")
...
def train_all_models(path, RESULTS):
    ...
    name = level + "_" + "R1_to_"+name + "_ZCAT_frozen"
...
def train_model(config, name,featureDim, feature_dims,
    ...
    pretrained_model = "G:/MEX/models/AE_LSTM_256_ZCAT0_F_R1_LSTM/final.pbz2"
    if config["C"] != "None":
        pretrained_model = pretrained_model.replace("AE", config["C"])

    pretrained_model = "G:/MEX/models/AE_MoE_256_ZCAT0_F_R1_ZCAT/final.pbz2"
    ...
    c_layer_2 = C(config=config, input_dims=[372]).cluster_model if C is not None else None
    pretrained_model = MoGen.load_checkpoint(filename=pretrained_model, Model=G, MiddleModel=c_layer_2)
    ...
    if C is not None:
        model.middle_layer = pretrained_model.middle_layer
        model.middle_layer.requires_grad_(False)
    model.cost_encoder = pretrained_model.cost_encoder
    ...
    try:
        for dname, dirs, files in os.walk("G:/MEX/datasets"):
            for i, file in enumerate(files):
                if "L" in file or "1_F" in file or "2_F" in file or "R1" in file:
                    continue
