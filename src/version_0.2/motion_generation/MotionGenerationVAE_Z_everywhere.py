...
class MotionGenerationModel(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        out = self.generationModel(embedding, x_tensors[0], pose_h)
    ...
    def step(self, x, y):
        ...
        for i in range(1, n):
            ...
            pos_cost_y = torch.mean(y[:, self.in_slices[-2]:self.posCost_dim]).detach()
            rot_cost_y = torch.mean(y[:, self.posCost_dim:]).detach()
