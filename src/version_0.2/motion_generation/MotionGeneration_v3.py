...
class MotionGenerationModel(pl.LightningModule):
    ...
    def forward(self, x, x_prev):
        ...
        x_tensors2 = [x_prev[:, d0:d1] for d0, d1 in zip(self.in_slices[:-1], self.in_slices[1:])]

        pose_h = self.middle_layer(self.pose_autoencoder.encode(x_tensors[1]))
        pose_h2 = self.middle_layer(self.pose_autoencoder.encode(x_tensors2[1]))

        embedding = torch.cat([pose_h, self.cost_encoder(x_tensors[2])], dim=1)
        embedding2 = torch.cat([pose_h2, self.cost_encoder(x_tensors2[2])], dim=1)

        out = self.generationModel(torch.cat((embedding, embedding2), dim=1), torch.cat((x_tensors[0], x_tensors2[0]), dim=1))
