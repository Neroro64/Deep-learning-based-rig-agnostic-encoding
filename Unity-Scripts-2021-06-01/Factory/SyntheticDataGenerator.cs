...
namespace Factory
{
    public class SyntheticDataGenerator : MonoBehaviour
    {
        ...
        public void CreateSequenceVersion2(Rig rig, Recorder recorder, bool randomRot, int num_targets, int k)
        {
            int[] id;
            if (num_targets > 1)
            {
                id = new int[num_targets];
                int kk;
                for (int i = 0; i < num_targets; ++i)
                {
                    kk = UnityEngine.Random.Range(0, 39);
                    foreach (var j in id)
                    {
                        if (kk == j)
                        {
        ...
        public void LoadSequence(Recorder recorder, string filepath)
        {
            var rawJson = DataIO.Load(filepath, prepend:false);
            var clip = Instantiate(recorder.prefabSequence);
            JsonUtility.FromJsonOverwrite(rawJson, clip);
            recorder.clip = clip;
        }
